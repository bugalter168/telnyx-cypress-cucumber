...
_(published from the `gh-pages` branch by CI — see [Reports](#reports). Replace
`bugalter168` once the repository is pushed.)_
...
| `npm run test-plan:generate`      | Regenerates `test-plan/telnyx-test-plan.xlsx`.                      |
...
The workbook is generated by `npm run test-plan:generate`. The `.docx` is maintained by hand and
is not touched by that script.
